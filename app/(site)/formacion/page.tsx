/**
 * PÁGINA DE FORMACIÓN — Cursos en innovación tecnológica
 */
...
function PostCard({ post }: { post: Post }) {
  const href = `/${post.seccion || 'formacion'}/${post.slug.current}`
  const date = new Date(post.publishedAt).toLocaleDateString('es-ES', { day: '2-digit', month: '2-digit', year: 'numeric' })
  const secLabel = SECCION_LABELS[post.seccion || 'formacion'] || 'Formación'
...
export default async function FormacionPage({ searchParams }: Props) {
  const { tematica, sector, page } = searchParams
  const allPosts = await getPostsBySeccion('formacion', { tematica, sector })
    ...
    <>
      <section className="bg-white">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 pt-16 pb-12 text-center">
          ...
          <h1 className="text-3xl md:text-4xl lg:text-5xl font-bold text-gray-900 leading-tight">Formación y cursos en<br />innovación tecnológica</h1>
          <p className="mt-5 text-gray-500 max-w-2xl mx-auto leading-relaxed">Formación especializada para empresas y profesionales del sector del plástico y el calzado.</p>
        </div>
      </section>
          ...
          <div className="flex-1 min-w-0">
            {posts.length > 0 ? (
              <>
                ...
                <Pagination currentPage={currentPage} totalPages={totalPages} basePath="/formacion" queryParams={{ ...(tematica ? { tematica } : {}), ...(sector ? { sector } : {}) }} />
              </>
            ) : (<p className="text-center text-gray-500 py-20">No hay programas de formación con estos filtros.</p>)}
          </div>
        </div>
      </div>
    </>
