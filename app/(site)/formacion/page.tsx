...
export default async function FormacionPage({ searchParams }: Props) {
  ...
  let allPosts = await getPostsBySeccion('formacion', { tematica, sector })
  if (q) {
    ...
    <>
      <section className="bg-white">
        <div className="max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8 pt-10">
          <SectionSearch basePath="/formacion" q={q} date={date} hiddenParams={{ tematica, sector }} />
        </div>
      </section>
      <section className="bg-white">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 pb-12 text-center">
          <h1 className="text-3xl md:text-4xl lg:text-5xl  text-gray-900 leading-tight">Formación y cursos en<br />innovación tecnológica</h1>
          <p className="mt-5 text-gray-500 max-w-2xl mx-auto leading-relaxed">En esta sección del Observatorio Tecnológico de CETEC se recopilan actividades formativas, cursos y programas de capacitación relacionados con la innovación, los materiales avanzados y el desarrollo tecnológico en los sectores del plástico y el calzado.</p>
        </div>
      </section>
            ...
            <div className="flex gap-4 overflow-x-auto pb-2">
              <div className="flex items-center gap-2 flex-shrink-0">
                ...
                {TEMATICAS.map((t) => (
                  <Link
                    key={t.value}
                    href={{ pathname: '/formacion', query: { ...(sector ? { sector } : {}), ...(tematica === t.value ? {} : { tematica: t.value }) } }}
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                ...
                {SECTORES.map((s) => (
                  <Link
                    key={s.value}
                    href={{ pathname: '/formacion', query: { ...(tematica ? { tematica } : {}), ...(sector === s.value ? {} : { sector: s.value }) } }}
