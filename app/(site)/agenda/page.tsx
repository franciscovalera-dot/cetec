/**
 * PÁGINA DE AGENDA — Eventos del sector
 */
...
function PostCard({ post }: { post: Post }) {
  const href = `/${post.seccion || 'agenda'}/${post.slug.current}`
  const date = new Date(post.publishedAt).toLocaleDateString('es-ES', { day: '2-digit', month: '2-digit', year: 'numeric' })
  const secLabel = SECCION_LABELS[post.seccion || 'agenda'] || 'Agenda'
  const secLabel2 = post.sector ? SECTOR_LABELS[post.sector] : null
  return (
    <Link href={href} className="group">
      <article className="bg-white rounded-2xl border border-gray-200 hover:shadow-lg transition-all h-full flex flex-col p-5">
        <div className="flex items-center gap-2 mb-4">
          {secLabel2 && <span className="text-[11px] font-semibold text-white px-3 py-1 rounded-full" style={{ background: 'linear-gradient(90deg, #FF813B 0%, #FFD4B8 100%)' }}>{secLabel2}</span>}
...
export default async function AgendaPage({ searchParams }: Props) {
  ...
  const posts = await getPostsBySeccion('agenda', { tematica, sector })
  return (
    <>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <div className="flex gap-10">
          ...
          <div className="flex-1 min-w-0">
            ...
            ) : (<p className="text-center text-gray-500 py-20">No hay eventos con estos filtros.</p>)}
          </div>
        </div>
      </div>
    </>
