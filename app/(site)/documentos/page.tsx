          ...
          <div className="lg:hidden w-full">
            <div className="flex gap-4 overflow-x-auto pb-2">
              <div className="flex items-center gap-2 flex-shrink-0">
                <span className="text-xs text-gray-900 uppercase">Tipo:</span>
                {TIPOS_DOCUMENTO.map((t) => (
                  <Link
                    key={t.value}
                    href={{ pathname: '/documentos', query: { ...(sector ? { sector } : {}), ...(tipo === t.value ? {} : { tipo: t.value }) } }}
                    className={`text-xs px-3 py-1.5 rounded-full whitespace-nowrap transition-colors ${tipo === t.value ? 'bg-orange-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-orange-50 hover:text-orange-600'}`}
                  >
                    {t.label}
                  </Link>
                ))}
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                ...
                {SECTORES.map((s) => (
                  <Link
                    key={s.value}
                    href={{ pathname: '/documentos', query: { ...(tipo ? { tipo } : {}), ...(sector === s.value ? {} : { sector: s.value }) } }}
