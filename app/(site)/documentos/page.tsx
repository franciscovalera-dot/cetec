  ...
  return (
    <a href={doc.file?.asset?.url || '#'} target="_blank" rel="noopener noreferrer" className="group">
      <article className="bg-white rounded-2xl border border-gray-200 hover:shadow-lg transition-all h-full flex flex-col p-5">
        ...
        <h3 className="font-bold text-gray-900 group-hover:text-orange-600 transition-colors line-clamp-2 leading-snug">{doc.title}</h3>
        {doc.description && <p className="text-sm text-gray-500 mt-2 line-clamp-3 leading-relaxed">{doc.description}</p>}
        {date && <time className="text-xs text-gray-400 mt-3 block">{date}</time>}
