...
export default async function NormativaPage({ searchParams }: Props) {
  ...
  let allPosts = await getPostsBySeccion('normativa', { tematica, sector })
  if (q) {
    ...
    <>
      ...
      <section className="bg-white">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 pb-12 text-center">
          <h1 className="text-3xl md:text-4xl lg:text-5xl  text-gray-900 leading-tight">Normativa y legislación del<br />sector del plástico y el calzado</h1>
          <p className="mt-5 text-gray-500 max-w-2xl mx-auto leading-relaxed">En esta sección del Observatorio Tecnológico de CETEC se recopilan las principales normativas, regulaciones y disposiciones legales que afectan a los sectores del plástico y el calzado.</p>
        </div>
      </section>
                ...
                <ul>
                  {TEMATICAS.map((t) => {
                    ...
                    return (
                      <li key={t.value}>
                        <Link
                          href={{ pathname: '/normativa', query: { ...(sector ? { sector } : {}), ...(active ? {} : { tematica: t.value }) } }}
                ...
                <ul>
                  {SECTORES.map((s) => {
                    ...
                    return (
                      <li key={s.value}>
                        <Link
                          href={{ pathname: '/normativa', query: { ...(tematica ? { tematica } : {}), ...(active ? {} : { sector: s.value }) } }}
                          className={`block text-sm py-2.5 pl-4 border-l-[3px] origin-left transition-all duration-200 ease-out hover:scale-105 hover:text-[#E8622A] hover:border-[#E8622A] ${active ? 'border-[#E8622A] text-[#E8622A] font-semibold' : 'border-[#E0E0E0] text-[#9E9E9E] font-normal'}`}
                        >
                          {s.label}
                        </Link>
            ...
            <div className="flex gap-4 overflow-x-auto pb-2">
              <div className="flex items-center gap-2 flex-shrink-0">
                ...
                {TEMATICAS.map((t) => (
                  <Link
                    key={t.value}
                    href={{ pathname: '/normativa', query: { ...(sector ? { sector } : {}), ...(tematica === t.value ? {} : { tematica: t.value }) } }}
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                ...
                {SECTORES.map((s) => (
                  <Link
                    key={s.value}
                    href={{ pathname: '/normativa', query: { ...(tematica ? { tematica } : {}), ...(sector === s.value ? {} : { sector: s.value }) } }}
