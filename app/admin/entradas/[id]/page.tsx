...
export default function EditarEntradaPage({ params }: { params: { id: string } }) {
  const { id } = params
  ...
  useEffect(() => {
    async function loadPost() {
      ...
      // Si tiene imagen, construir URL de preview
      if (post.image?.asset?._ref) {
        ...
        if (parts.length >= 3) {
          ...
          setImagePreview(
            `https://cdn.sanity.io/images/${projectId}/${dataset}/${imgId}.${format}`
          )
        }
      }
    ...
    return (
      <div className="min-h-screen bg-gray-100 flex items-center justify-center">
        <div className="flex items-center gap-3">
          ...
          <span className="text-sm text-gray-900">Cargando entrada...</span>
        </div>
      </div>
    )
  }
      ...
      <header className="bg-white border-b border-gray-200 sticky top-0 z-30">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-lg  text-gray-900">Editar entrada</h1>
          </div>
